...
Options
    ...
    -i	      if prepare_atmospherics.sh is running, just ignore it
...
if [ "$rep" == "true" ] && ! [ -s $this ] ; then
	...
	echo "Please repair manually or resubmit full prepare_atmospherics.sh"
	exit 1
fi
...
for rr in "${repair[@]}" ; do
	...
	$generate $Atmo $Name $njobs $this $outlog $rr > $recover
	$sub $recover
done
