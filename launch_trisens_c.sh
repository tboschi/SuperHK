...
model=( "11a" "11b" )
#model=( "new0_flux" "flux_lukas_flux" )
...
#while [ $count -lt $lengt ] ; do
	...
	echo $sens $fop -g $study/longglobal/$point -r $study/$mod/$point -1 NH -2 NH
	$sens $fop -g $study/longglobal/$point -r $study/$mod/$point -1 NH -2 NH

	while [ $(condor_q -run -format "%s\n" cmd | grep fitter | wc -l) -gt 50 ] ; do
		echo 'launcher: waiting for jobs to end...(10 min)'
		sleep 600
	done
done
