...
int main(int argc, char** argv)
{
	...
	//create chi2 fitter
	ChiSquared *fitter = new ChiSquared(cd, 2224);
	fitter->Init();
	...
	std::cout << "getting entry " << tPoint << std::endl;
	...
	Eigen::VectorXd trueSpectra = fitter->LoadSpectrum(tPoint, "true_input");
	...
	for (int i = nstart; i < nend; ++i)
	{
		...
		Eigen::VectorXd fitSpectra = fitter->LoadSpectrum(Point, "fit_input");
