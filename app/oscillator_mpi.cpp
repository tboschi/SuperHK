...
int main(int argc, char** argv)
{
	...
	std::cout << "Loading " << fileElements->GetEntriesFast() << " files\n";
	TIter next(fileElements);
	TChainElement *current = 0;
	int nentry = 0;
	ch->GetEntry(nentry);	//load first entry to compute nentries

#pragma omp parallel for default(none) shared(std::cout, std::endl, outname, fileElements, ch, nBinAtm, nBinBeam)
	for (int f = 0; f < fileList.size(); ++f)
	{
		...
		double _dCP = -9999;
		int evt = current->GetEntries();
		std::cout << "Number of entries: " << nentry << " to " << nentry+evt << std::endl;
		std::cout << "thr " << omp_get_thread_num() << std::endl;
		for (int n = nentry; n < nentry + evt; ++n)
		{
			std::cout << "_";
			ch->GetEntry(n);
			...
			if (MH == "inverted")
			{
				oscHK->SetMasses<Oscillator::inverted>(M12, M23);
				oscKD->SetMasses<Oscillator::inverted>(M12, M23);
			}
			else
			{
