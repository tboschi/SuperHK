...
int main(int argc, char** argv)
{
	...
	std::ofstream outPro(prob3.c_str());
	...
	prob->SetInverseMassHierarchy(true);
	prob->SetBaseLine(baseline);
	prob->SetDensity(density);
	prob->GetProbNuMuNuMu(1);
	...
	{
		if (anti)
			prob->SetAntiNeutrino(1);
		else
			prob->SetAntiNeutrino(0);
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				mat << prob->GetProb(i+1, j+1) << "\t";
				outPro << "\t" << prob->GetProb(i+1, j+1);
			}
			mat << "\n";
		}
		mat << "\n\n";
